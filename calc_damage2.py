# -*- coding: utf-8 -*-
"""
Created on Mon Aug 23 15:32:43 2021

@author: aeapo
"""

def calc_damagex2(character,weapon, artifacts,epe):
    attack = character['attackc']
    em = character['emc']
    er = character['erc']
    cr = character['crc']
    cd = character['cdc']
    ...
    if epe['epe']=='geo':
        ...
        electro_particle_boost=0.5
    if epe['epe']=='cryo' or epe['epe2']=='cryo':
        cr+=0.15
    thp=hp
    tdf=df
    ta=attack
    ...
    if character['element reaction'] == 'vaporise' or character['element reaction']=='melt':
        ...
        if character['hp bonus_damage']:
            ...
            if character['play_style'] == 'main dps':
                ...
                Q_recharge = character['Q_cost']/(pps*(1+er))
                if Q_recharge<character['Q_cd']:
                    dps = y*(number_NA*(character['NA']*na_mult+naca_bonus) + number_CA*(character['CA']*ca_mult+naca_bonus) + number_E*(character['E']*e_mult+e_bonus) + number_Q*(character['Q']*q_mult+q_bonus))+(number_NA+number_CA+number_E+number_Q)*x*em_bonus
                else:
                    dps = y*(number_NA*(character['NA']*na_mult+naca_bonus) + number_CA*(character['CA']*ca_mult+naca_bonus) + number_E*(character['E']*e_mult+e_bonus) + number_Q*(character['Q']*(character['Q_cd']/Q_recharge)*q_mult+q_bonus))+(number_NA+number_CA+number_E+number_Q)*x*em_bonus
                return dps
            if character['play_style'] == 'quickswap':
                number_NA = int(character['QS'][0])
                number_CA = int(character['QS'][1])
        ...
        if character['play_style'] == 'main dps':
            ...
            Q_recharge = character['Q_cost']/(pps*(1+er))
            if Q_recharge<character['Q_cd']:
                dps = y*(number_NA*character['NA']*na_mult + number_CA*character['CA']*ca_mult + number_E*character['E']*e_mult + number_Q*character['Q']*q_mult)+(number_NA+number_CA+number_E+number_Q)*x*em_bonus
            else:
                dps = y*(number_NA*character['NA']*na_mult + number_CA*character['CA']*ca_mult + number_E*character['E']*e_mult + number_Q*character['Q']*(character['Q_cd']/Q_recharge)*q_mult)+(number_NA+number_CA+number_E+number_Q)*x*em_bonus
            return dps
        if character['play_style'] == 'quickswap':
            number_NA = int(character['QS'][0])
            number_CA = int(character['QS'][1])
